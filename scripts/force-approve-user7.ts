// Force approve KYC user with proper partition key handling
...
async function forceApprove() {
    ...
    // Ensure partition key field exists (userId)
    if (!user.userId) {
        user.userId = user.id;
    }
    const updatedUser = {
    ...
    console.log('User approved successfully via upsert with proper partition key.');
}
