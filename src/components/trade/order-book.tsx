...
function groupOrders(orders: RawOrder[], agg: number): Order[] {
  ...
  for (const [priceStr, amountStr] of orders) {
    ...
    const groupedPrice = Math.round(price / agg) * agg; // nearest bucket
...
export function OrderBook({
  ...
  const groupedAsks = useMemo(() => {
    return groupOrders(asks, aggregation).sort((a, b) => b.price - a.price); // descending for rendering
  }, [asks, aggregation]);

  const groupedBids = useMemo(() => {
    return groupOrders(bids, aggregation).sort((a, b) => b.price - a.price);
  }, [bids, aggregation]);

  // max total for heatmap scale
  ...
  // auto-center to mid price when data updates (if not locked)
  useEffect(() => {
    ...
    ({ index, style, data }: { index: number; style: React.CSSProperties, data: { items: Order[], side: 'bid' | 'ask'} }) => {
      ...
      return (
        <div
          ...
          style={style}
          className={cn("px-2 flex justify-between items-center text-xs relative hover:bg-muted font-mono")}
        >
  ...
  return (
    <Card>
      ...
      <CardContent className="p-4 pt-0">
        ...
        <div className="h-[400px] overflow-y-hidden" style={{height: `${height}px`}}>
            <List
                height={height / 2 - 20}
                itemCount={groupedAsks.length}
                itemSize={rowHeight}
                width={"100%"}
                ref={asksListRef}
                itemData={{items: groupedAsks, side: 'ask'}}
                >
                {Row}
            </List>
            ...
            <List
                height={height / 2 - 20}
                itemCount={groupedBids.length}
                itemSize={rowHeight}
                width={"100%"}
                ref={bidsListRef}
                itemData={{items: groupedBids, side: 'bid'}}
                >
                {Row}
            </List>
        </div>
