...
export function Balances({ marketId }: { marketId: string }) {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Error</AlertTitle><AlertDescription>Failed to load balances.</AlertDescription></Alert>;
    }
    ...
    return (
      <div className="space-y-2 text-sm">
        <div className="flex justify-between"><span className="text-muted-foreground">{baseAsset.symbol} Balance</span><span className="font-mono font-medium">{(baseBalance?.available ?? 0).toFixed(6)}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">{quoteAsset.symbol} Balance</span><span className="font-mono font-medium">{(quoteBalance?.available ?? 0).toFixed(2)}</span></div>
      </div>
    );
  };
