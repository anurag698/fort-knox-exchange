...
export default function MarketHeader({ symbol, ticker }: Props) {
  const base = symbol.slice(0, symbol.length - 4).toUpperCase();
  const quote = symbol.slice(symbol.length - 4).toUpperCase();

  const price = ticker?.c ? parseFloat(ticker.c).toFixed(2) : "--";
  const change = ticker?.P ? `${parseFloat(ticker.P).toFixed(2)}%` : "--";

  const isUp = ticker?.P && parseFloat(ticker.P) >= 0;

  return (
    <div className="flex items-center justify-between bg-[#111418] p-3 rounded-xl">

      {/* LEFT SIDE: PAIR + PRICE */}
