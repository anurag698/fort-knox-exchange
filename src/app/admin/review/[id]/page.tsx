...
function ReviewWithdrawalClient({ reviewId }: { reviewId: string }) {
  const { data: withdrawal, isLoading: withdrawalLoading, error: withdrawalError } = useWithdrawal(reviewId);
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2">KYC Status: <Badge variant={userProfile?.kycStatus === 'approved' ? 'default' : 'secondary'}>{userProfile?.kycStatus}</Badge></p>
        </div>
        <div>
...
export default async function ReviewWithdrawalPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  return <ReviewWithdrawalClient reviewId={id} />;
}
