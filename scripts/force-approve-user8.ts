// Force approve KYC user with correct partition key handling using CosmosClient
...
async function forceApprove() {
    ...
    // Use partition key (userId) for replace
    await container.item(updatedUser.id, updatedUser.userId).replace(updatedUser);
    console.log('User approved successfully via replace with correct partition key.');
}
